            ...
            if (status == AugmentedTarget::kTargetStatusTracked) {
                ...
                if (!tracked_target) {
                    if (video == NULL) {
                        ...
                        else if (frame.targets()[0].target().name() == std::string("namecard") &&
                                 texid[1]) {
                            video = new ARVideo;
                            video->openTransparentVideoFile("transparentvideo.mp4", texid[1]);
                            video_renderer = renderer[1];
                        }
                        else if (frame.targets()[0].target().name() == std::string("idback") &&
                                 texid[2]) {
                            video = new ARVideo;
                            video->openStreamingVideo(
                                    "http://7xl1ve.media1.z0.glb.clouddn.com/sdkvideo/EasyARSDKShow201520.mp4",
                                    texid[2]);
